...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "amount of guidance to use for mixing the outputs from autoregressive and direct inference"
        ),
    )
    parser.add_argument(
        "--scale_factor",
...
def main(args):
    ...
    sampler_dict = {"ddpm": DDPMScheduler, "ddim": DDIMScheduler, "dpm": DPMSolverMultistepScheduler}
    ...
    resultsfile = open(f"{args.model_dir}/checkpoint-{args.checkpoint_number}/results_{args.sampler}_mixguidance{args.mix_guidance}.txt", "w")
    ...
    for b, batch in enumerate(eval_dataloader):
        ...
        for i in range(20):
            if not args.evaluate_interpolation:
                ...
                assert args.model_type == "clippose"
                if i == 0:
                    direct_input = (data_point, plucker)
                    plucker_ar = plucker.clone()
                    plucker_ar[:, 3, :] = 0.5
                    autoregressive_input = (data_point, plucker_ar)
                else:
                    last_output = torch.stack([image_transforms(Image.fromarray((last_output[i] * 255.0).astype("uint8"))).to(data_point.device) for i in range(batch_size)])
                    data_point_ar = torch.cat([direct_input[0][:, 1:-1, ...], last_output[:, None, ...], last_output[:, None, ...]], dim=1)
                    autoregressive_input = (data_point_ar, plucker_ar)

                data_point_combined = torch.cat([direct_input[0], autoregressive_input[0]], axis=0)
                plucker_combined = torch.cat([direct_input[1], autoregressive_input[1]], axis=0)

                prediction, _ = pipeline(
                            inpainting_image=(data_point_combined, plucker_combined),
                            batch_size=batch_size,
                            guidance=args.guidance,
                            mix_guidance=args.mix_guidance,
                ...
                last_output = prediction[:, 0, 0, ...].numpy()
                # print(last_output[0, ...].shape)
                # plt.imsave(f"/data3/tkhurana/autoregressive_output_{i}.png", last_output[0, ...])
        ...
        if not args.model_type in ["img2img", "clippose"]:
            ...
            prediction = prediction.reshape(B, 1, C, H, W)
            prediction = torch.cat([direct_input[0][:, :args.num_images, :, :, :].cpu(), prediction], axis=1)
        ...
        if args.model_type in ["img2img", "clippose"]:
            assert data_point_combined.ndim == 5
        else:
            assert data_point_combined.ndim == 4
        groundtruth = direct_input[0].cpu()

        # prediction = prediction / 2 + 0.5
        groundtruth = groundtruth / 2 + 0.5

        print("shape of prediction", prediction.shape, prediction.min(), prediction.max())
        print("shape of groundtruth", groundtruth.shape, groundtruth.min(), groundtruth.max())
        ...
        if "d" in args.data_format:
            ...
            print("min omax of drpth", top1_prediction.min(), top1_prediction.max(), top1_groundtruth.min(), top1_groundtruth.max())
        ...
        if "rgb" in args.data_format:
            ...
            top1_prediction, top1_groundtruth = return_luminance(top1_prediction, top1_groundtruth)
            print("unique values in pred and gt", np.unique(top1_prediction), np.unique(top1_groundtruth))
            ...
            top3_prediction, top3_groundtruth = return_luminance(top3_prediction, top3_groundtruth)
            ...
            top5_prediction, top5_groundtruth = return_luminance(top5_prediction, top5_groundtruth)
