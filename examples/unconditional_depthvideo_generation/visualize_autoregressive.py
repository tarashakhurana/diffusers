            ...
            for b in range(prediction.shape[0]):
                ...
                for fn in range(prediction.shape[1]):
                    ...
                    if "d" in args.data_format:
                        ...
                        os.makedirs(f"{args.model_dir}/checkpoint-{args.checkpoint_number}/train_ddpm_autoregressive_multifps/{seq}_{args.output_fps}_depth{suffix}", exist_ok=True)
                        if fn == args.num_images:
                            path = f"{args.model_dir}/checkpoint-{args.checkpoint_number}/train_ddpm_autoregressive_multifps/{seq}_{args.output_fps}_depth{suffix}/{framenumber:0>5}_3.png"
                            ...
                            path = f"{args.model_dir}/checkpoint-{args.checkpoint_number}/train_ddpm_autoregressive_multifps/{seq}_{args.output_fps}_depth{suffix}/groundtruth_{framenumber:0>5}_3.png"
                            ...
                            groundtruth = cmap(groundtruth)
                            groundtruth = Image.fromarray((groundtruth * 255).astype(np.uint8))
                            path = f"{args.model_dir}/checkpoint-{args.checkpoint_number}/train_ddpm_autoregressive_multifps/{seq}_{args.output_fps}_depth{suffix}/input_{framenumber:0>5}_{fn}.png"
                            if framenumber == 0:
                                groundtruth.save(path)
