...
@dataclass
class UNet4DVoxelOutput(BaseOutput):
    """
    Args:
...
class UNet4DVoxelModel(ModelMixin, ConfigMixin):
    r"""
    UNet4DVoxelModel is a 2D UNet model that takes in a noisy sample and a timestep and returns sample shaped output.
    ...
    def __init__(
        ...
        #### argument masking_strategy is not used anywhere but exists for consistency with other voxel models
        ...
        self.input_output_grid = [self.n_output + self.n_input, self.n_height, self.n_length, self.n_width]
    ...
    def forward(
        ...
        assert T_total == self.n_input + self.n_output and H == self.n_height

        clean_4dvoxel_masked = total_occupancy
        clean_4dvoxel_masked[:, self.n_input:, ...] = torch.zeros_like(
                clean_4dvoxel_masked[:, self.n_input:, ...],
                device=clean_4dvoxel_masked.device)
        inpainting_mask = torch.zeros_like(clean_4dvoxel_masked, device=clean_4dvoxel_masked.device)
        inpainting_mask[:, self.n_input:, ...] = torch.ones_like(
                inpainting_mask[:, self.n_input:, ...],
                device=inpainting_mask.device)

        # Unnormalize the points, int-fy them and initialize a noisy voxel grid
        ...
        inpainting_mask = inpainting_mask.reshape(N, -1, L, W)

        # Predict the noise residual
        model_inputs = torch.cat([noisy_4dvoxel, clean_4dvoxel_masked, inpainting_mask], dim=1)
        ...
        return UNet4DVoxelOutput(sample=ret_dict)
