...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Amount of guidance at inference for mixing direct and autoregressive inference"
        ),
    )
    parser.add_argument(
        "--scale_factor",
    ...
    for batchidx in iterable_array:
        ...
        for framenumber in range(args.num_autoregressive_frames):
            ...
            if last_input == None or args.use_groundtruth:
                ...
                plucker[0][3][0] = 1 / args.output_fps
                last_input = (data_point, rgb_images, plucker)
                first_input = (data_point, rgb_images, plucker)
                ...
                future_plucker = torch.Tensor([1 / args.output_fps]).to(plucker.device) + plucker[0, -1, 0]
                if dp is not None:
                ...
                plucker = torch.cat([plucker[:, 1:, :], future_plucker[None, :, None]], dim=1)
                plucker = plucker - plucker[0, 2, 0]
                last_input = (data_point, rgb_images, plucker)
                #####################################################################################################
            ...
            if args.model_type == "inpainting":
                ...
                # construct the batch_size = 2 input which would contain the inputs
                # for both the autoregressive inference and direct inference
                combined_data_point = torch.cat([first_input[0], last_input[0]])
                first_plucker = first_input[2]
                first_plucker[0][3][0] += 1 / args.output_fps
                combined_plucker = torch.cat([first_plucker, last_input[2]])

                print(combined_plucker)

                prediction, _ = pipeline(
                    inpainting_image=(combined_data_point, combined_plucker),
                    batch_size=batch_size,
            ...
            for b in range(prediction.shape[0]):
                ...
                for fn in range(prediction.shape[1]):
                    ...
                    if "d" in args.data_format:
                        ...
                        os.makedirs(f"{args.model_dir}/checkpoint-{args.checkpoint_number}/train_ddpm_autoregressive_multihyp_jan23_run4/{seq}_{args.output_fps}_depth{suffix}", exist_ok=True)
                        if fn == args.num_images:
                            path = f"{args.model_dir}/checkpoint-{args.checkpoint_number}/train_ddpm_autoregressive_multihyp_jan23_run4/{seq}_{args.output_fps}_depth{suffix}/{framenumber:0>5}_3.png"
                            ...
                            path = f"{args.model_dir}/checkpoint-{args.checkpoint_number}/train_ddpm_autoregressive_multihyp_jan23_run4/{seq}_{args.output_fps}_depth{suffix}/groundtruth_{framenumber:0>5}_3.png"
                            ...
                            groundtruth = cmap(groundtruth)
                            groundtruth = Image.fromarray((groundtruth * 255).astype(np.uint8))
                            path = f"{args.model_dir}/checkpoint-{args.checkpoint_number}/train_ddpm_autoregressive_multihyp_jan23_run4/{seq}_{args.output_fps}_depth{suffix}/input_{framenumber:0>5}_{fn}.png"
                            if framenumber == 0:
                                groundtruth.save(path)
