...
from data import TAOMAEDataset, DeformingThings4DDataset
...
def main():
    ...
    for name, para in unet.named_parameters():
        ...
        if 'conditioning_compressor' in name:
            scratch_parameters_list.append(para)
            para.requires_grad = True
    optimizer = optimizer_cls(
        [{"params": parameters_list},
         {"params": scratch_parameters_list, "lr": args.learning_rate * 10}],
    ...
    train_dataset = DeformingThings4DDataset(
        instance_data_root="/data/tkhurana/DeformingThings4D/",
        size=256,
        center_crop=False,
    ...
    )  # number of frames returned should be args.num_frames + 1
    ...
    def encode_image(pixel_values):
        ...
        print(image_embeddings.shape, "image emb shape")
        image_embeddings = image_embeddings.reshape(B, N, -1)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                conditional_pixel_values = pixel_values[:, :args.num_frames, :, :, :]

                latents = tensor_to_vae_latent(pixel_values[:, args.num_frames:args.num_frames + 1, :, :, :], vae)
                ...
                encoder_hidden_states = encode_image(
                    pixel_values[:, :args.num_frames, :, :, :].float())
                ...
                inp_noisy_latents = inp_noisy_latents.repeat(1, conditional_latents.shape[1], 1, 1, 1)
                ...
                # MSE loss
                loss = torch.mean(
                    (weighing.float() * (denoised_latents[:, -2:-1, :, :, :].float() -
